...
* **출원/등록 상태 우선순위 자동 판별:** 출원번호와 등록번호를 분석하여, 둘 다 있으면 등록번호 우선으로 기입하고 상태 열에 "등록"으로 처리합니다.
* **담당자 열(CZ열) 매핑:** 기술이전 산학협력단 담당자의 이름이 V~Z열을 건너뛰고 CZ열의 `담당자` 칸에 정확히 들어갑니다.
* **표2 요약 추출 강화:** [표2]의 수납상황 정보가 `업체명(비율) (담당자명, 이메일/전화번호)` 형태로 요약되어 **91.수납상황** 열에 기재됩니다.
* **연구과제 추출 강화:** '연구기간', '연구개발비 내 계(총연구비)', '연구책임자' 항목이 훨씬 더 정확하게 타겟팅되어 추출됩니다.
* **지역구분 세분화:** '경상남도', '경기도', '서울특별시' 등 도/광역시/특별시 명칭이 주어져도 올바르게 '055 경남' 등으로 매핑됩니다.
* **날짜 및 금액 서식:** 엑셀 날짜 형식(복붙 시 완벽 호환)과 천 단위 콤마(,) 회계 형식이 모두 적용됩니다.
""")
...
st.markdown("---")
# 복사/붙여넣기 편의를 위한 시작 연번 지정 기능 추가
start_seq_num = st.number_input("🔢 시작 연번 지정 (기존 데이터베이스의 마지막 연번 다음 번호를 입력하세요)", min_value=1, value=1, step=1)
...
if st.button("🚀 대량 데이터 추출 시작", use_container_width=True):
    if not contract_files:
        ...
        for idx, d in enumerate(all_extracted_data):
            ...
            # 지정한 시작 연번을 반영하여 순차적으로 번호 부여
            row_dict["1.연번"] = start_seq_num + idx
